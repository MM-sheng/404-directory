...
After the v0.10.2 production cutover, a clean client install must list all
...
| ------------------------ | --------------------------------------------- | ------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- | ------------------------------- |
| Universal stdio          | `npx -y @mmvv1638/404-directory-mcp`          | Package, identity persistence, concurrency, protocol negotiation and forwarding tests | Clean install against public npm v0.10.2 and run one risk preflight           | `npx` or supplied `--source`    |
| Official MCP Registry    | npm package plus remote HTTP in `server.json` | Registry validation and package ownership metadata                                    | Install from a Registry-compatible client after v0.10.2 propagation           | `official-registry`             |
...
| 2026-08-24 | Generic Streamable HTTP / MCP SDK 1.30.0   | Production `https://404.directory/mcp` after v0.9.3 rollout                             | `release-smoke`                                              | Yes, exactly 12                                                                        | `verify_web`, Microsoft `search_official_docs`, and `search_tools` returned non-error structured content                 | Stable internal identities; explicitly excluded from adoption                            | Internal          | Cloud Run revision `directory-404-v093-2396798`, release workflow and local production smoke output | Pass against production v0.9.3; all 3 MCP prompts also listed and rendered                                                                          |
| 2026-08-29 | Codex CLI 0.150.0-alpha.12.2               | Existing global stdio acceptance config                                                 | `codex.acceptance`                                           | Not rerun: the installed acceptance entry is still pinned to public npm v0.9.2         | Not rerun; no model call was made                                                                                        | Existing bridge configuration remains enabled                                            | Internal          | Read-only `codex mcp list`                                                                          | Blocked for current acceptance: update the dedicated acceptance entry to the reviewed release, then run a real task without counting it as adoption |
| 2026-08-29 | Claude Code 2.1.205                        | Existing production HTTP entry plus packaged plugin                                     | `claude.acceptance` / `codex-plugin`                         | Direct HTTP entry reports connected; packaged plugin reports pending approval          | Not run; no paid/model call or approval was performed                                                                    | Direct path and plugin configuration retain their existing identity setup                | Internal          | Read-only `claude mcp list`                                                                         | Partial: transport health passed; plugin approval, current inventory and one useful native call remain unverified                                   |
| 2026-08-29 | Cursor                                     | No callable Cursor CLI in the audit shell                                               | —                                                            | Not verified                                                                           | Not verified                                                                                                             | Not verified                                                                             | Internal          | Read-only executable discovery                                                                      | Blocked: use a clean Cursor Agent profile after release, confirm current inventory, then complete one real preflight                                |

The 2026-08-29 Skill update is locally validated against its real MCP schema and
package mirror. Its bridge test forwards an exact `evaluate_prediction_market`
call with stable identity, source, client family and negotiated protocol. A
separate local MCP client completed an evidence-backed `observe` preflight over
deterministic public fixtures without fetching an order book or executing a
trade. These are release tests, not evidence that a native model selected the
Skill autonomously.
